<?php
    ...
    try {
      ...
      $restaurantDishes = $db->prepare("select * from dish where rid = ?");
      $restaurantDishes->execute(array($arr[2]));
      $innerSql = "where did IN (";
      $i = 1;
      $count = $restaurantDishes->rowCount();
      while ($row = $restaurantDishes->fetch(PDO::FETCH_OBJ)) {
        $innerSql .= $row->DID;
        if($i != $count){
          $innerSql .= ",";
        }
      ...
      $innerSql .= ")";
      $sql = "select * from dishreviews $innerSql limit $p1,10";
  ...
  <body>
    ...
    <div class="container">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Dish Comments</li>
      </ol>
      <!-- Portfolio Item Row -->
      <div class='row'>
          <?php
	            ...
	            while($row = $reviews->fetch(PDO::FETCH_OBJ)){
                  ...
                  <form name="sentMessage" id="commentForm" method="post" enctype="multipart/form-data" action="scripts/replayDish.php">
                  <div class='row'>
                    <div class='col-md-12'>
                      <?php
