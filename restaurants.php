<?php
  ...
  try {
      require('scripts/connection.php');
			//get all types
			$allTypes = $db->prepare("select DISTINCT type from restaurantstypes order by type asc");
			$allTypes->execute();
			//get all branch locations
			$allLocations = $db->prepare("select DISTINCT location from branch order by location asc");
			...
			//end filter types
      $sql = "select * from restaurants where restaurantname like ? $ridInSql $ridInSqlBranch and rating >= ? $orderby limit $p1,10";
      $stmt = $db->prepare($sql);
      ...
      <div class='row'>
        ...
        <!-- Portfolio Item Row -->
	      ...
	      <?php
				...
				echo "<h4 style='color:red;'>No restaurants</h4>";
			}
    ?>
    </div>
		<div class='col-md-3'>
